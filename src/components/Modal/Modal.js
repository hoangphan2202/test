...
export default function Modal({ open, onClose, children, size, className, background = 'white', classNameContainer }) {
          ...
          >
            <div
              className={classnames(
                ...
                size === 'sm' && 'sm:max-w-md',
                size === 'md' && '!max-w-screen-lg',
                className,
              )}
            >
              <div className={classnames(`bg-${background}`, 'p-2 sm:p-4', classNameContainer)}>{children}</div>
            </div>
          </Transition.Child>
        </div>
